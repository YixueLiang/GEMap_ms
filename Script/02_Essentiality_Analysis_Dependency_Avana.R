...
# Load the data
load("DEMap_Data/Avana_DATA.Rdata")
load("DEMap_Data/CRISPR_sanger_avana_gecko.Rdata")

# Data preprocessing
# rownames(df_Avana_cell) <- df_Avana_cell$CellName
commoncell <- intersect(rownames(df_Avana), df_cell$StrippedCellLineName)
...
a <- Tissue_groups[["CNS"]]
Tissue_groups[["CNS"]] <- c(a, "D341MED")

# Create Avana_groups by splitting df_Avana based on tissue groups
Avana_groups <- list()

for (Tissue in names(Tissue_groups)) {
  Avana_groups[[Tissue]] <- df_Avana[Tissue_groups[[Tissue]], ]
}
...
for (Tissue in names(Avana_groups)) {
  gene_values[[Tissue]] <- Avana_groups[[Tissue]]["CDK4"]
}
...
for (Tissue in names(gene_values)) {
  ...
  Avana_vectors[[Tissue]] <- vector_df
}
...
for (Tissue in names(Avana_vectors)) {
  temp_df <- data.frame(
    Value = Avana_vectors[[Tissue]],
    Tissue = Tissue,
...
colors <- colorRampPalette(rev(brewer.pal(9, "RdYlBu")))(27)
