...
ampCells = cnGeneData$DepMap_ID[cnGeneData$cnGene > 1.6]
wtCells = cnGeneData$DepMap_ID[cnGeneData$cnGene <= 1.6]
...
save(data, file = "inputdata/two_rank/data_ccne1.RData")

data_sorted <- data[order(data$rank), ]
data_sorted <- data_sorted[-1, ]
data_sorted <- data_sorted[1:300, ]

write.csv(data_sorted, "CCNE1_300_NP.csv", row.names = FALSE)
...
# write.csv(mutData, "mutData.csv", row.names = T)

com <- intersect(rownames(df_crispr), rownames(mutData))
...
data$rank <- -log10(data$Wilcox_p_value)*data$dep.FC
data <- data[order(data$rank), ]
save(data, file = "inputdata/two_rank/data_kras.RData")

data_sorted <- data[order(data$dep.FC), ]
data_sorted <- data_sorted[-1, ]
data_sorted <- data_sorted[1:300, ]

write.csv(data_sorted, "KRAS_300_NP.csv", row.names = FALSE)
...
ggplot(data, aes(dep.FC, -log10(Wilcox_p_value))) +
  ...
  xlab(expression(paste(Delta,"FC (KRAS MUT vs WT)"))) +
  ylab(expression(paste("-log"[10], "(Wilcoxon ", italic("P"), " value)"))) +
  labs(title = "DepMap Dependencies", subtitle = "KRAS-MUT Tumor Cell Lines") +
  theme(
...
combined_data <- rbind(
  data.frame(Value = mutCellsDepData$POLR1C, Group = "MUT"),
  data.frame(Value = wtCellsDepData$POLR1C, Group = "WT")
)
