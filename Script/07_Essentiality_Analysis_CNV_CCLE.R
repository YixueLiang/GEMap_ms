...
# df_cn <- df_cn[com, ]
commoncell <- intersect(rownames(df_cn), df_cell$ModelID)
df_cn <- df_cn[commoncell, ]
rownames(df_cell) <- df_cell$ModelID
df_cell <- df_cell[rownames(df_cn), ]
rownames(df_cn) <- df_cell$StrippedCellLineName
...
# Create crispr_groups by splitting df_cn based on tissue groups
crispr_groups <- list()

for (Tissue in names(Tissue_groups)) {
  crispr_groups[[Tissue]] <- df_cn[Tissue_groups[[Tissue]], ]
}
...
ggplot(plotdata, aes(x = Tissue, y = Value, fill = Tissue)) +
  ...
  labs(x = "Tissue", y = "CNV", title = paste0(gene, " — Primary Disease"))
