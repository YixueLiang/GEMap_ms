...
com_target <- intersect(depmap_23q2, lincs)
...
data <- subset(data, GeneSymbol != "KRAS")
...
ggplot(data, aes(dep.FC, -log10(Wilcox_p_value))) +
  geom_point(shape = 21, colour = "gray30", aes(fill = -log10(Wilcox_p_value)), size = 3) + 
  scale_fill_gradient(low = "#9ECFE3", high = "#FB9F5A") +
  geom_point(data = subset(data, GeneSymbol %in% genes_to_show), 
             shape = 21, colour = "gray30", fill = "#F0663F", size = 3, stroke = 1.2) +
  geom_text_repel(data = subset(data, GeneSymbol %in% genes_to_show), 
                  aes(label = GeneSymbol), size = 5, fontface = "bold", color = "black", 
  ...
  theme(
    ...
    axis.title = element_text(size = 14, face = "bold"),  # 加粗 x 和 y 轴标题
    axis.text = element_text(size = 12),
    legend.position = "right", 
...
combined_data <- rbind(
  data.frame(Value = mutCellsDepData$DOCK5, Group = "MUT"),
  data.frame(Value = wtCellsDepData$DOCK5, Group = "WT")
)
...
ggplot(combined_data, aes(x = Group, y = Value, fill = Group)) +
  ...
  labs(x = "MUT Status", y = "DOCK5 Value", 
  ...
  theme(
    ...
    panel.border = element_rect(colour = "black", fill = NA, size = 0.8),  # Add rectangular border
    legend.position = "none",  # Hide legend if not needed
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),  # Centered plot title with bold font
    plot.background = element_blank()  # Ensure clean white background
  ) +
  ...
  coord_cartesian(clip = "off")  # Ensure border is fully visible
